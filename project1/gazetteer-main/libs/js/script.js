  ...
  L.easyButton('fa-virus', function () {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if(data === null){
            console.log("You have not selected a country");
        } else {
        data.pop();
        }
          
        if (result.status.name == 'ok') {
          $('#txtConfirmed').html(roundData(data.pop().Confirmed));
          $('#txtActive').html(roundData(data.pop().Active));
          $('#txtRecovered').html(roundData(data.pop().Recovered));
          $('#txtDeaths').html(roundData(data.pop().Deaths));
        }
      }
    })
  ...
  L.easyButton('fa-money-bill', function () {
    ...
    window.onclick = function (event) {
     ...
     L.easyButton('fa-home', function () {
        let locationsd = $('#countrySelect option:selected').val();
    ...
    var displayRibbon = L.ExtraMarkers.icon({
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            ribbonDatas.forEach(function(ribbonDatas) {
                ...
                var dynamicRibbons = L.ExtraMarkers.icon({
